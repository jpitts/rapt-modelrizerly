...
RedisDS.prototype.read = function(id, model, cb) {
	...
	var bound_cb = this.bind_model_to_cb(model, cb)
		, id = id || model[model.pk_name()]

  model.log_start_block({ classname:'modelrizerly.datastores.mongodb', block:'read', 'subject_id':id });
...
RedisDS.prototype.update = function update(opts, model, cb) {
  model.log_start_block({ classname:'modelrizerly.datastores.mongodb', block:'update' });
	...
	this.client.hmset(model[model.pk_name()], serialize(data), function(err) {
		return err ? cb(err) : cb(err, model);
	});
}
